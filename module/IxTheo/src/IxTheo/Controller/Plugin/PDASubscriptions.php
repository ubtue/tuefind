...
class PDASubscriptions extends AbstractPlugin
{
    ...
    public function getUserData(UserEntityInterface $user)
    {
    ...
    public function constructAddress($emailAddressString, $emailName = '')
    {
     ...
     */
    public function sendEmail($recipientEmail, $recipientName, $senderEmail, $senderName, $emailSubject, $emailMessage)
    {
        try {
            $mailer = $this->mailer;
     ...
     */
    public function sendPDANotificationEmail($post, UserEntityInterface $user, $data, $id)
    {
     ...
     */
    public function getPDASenderData($realm)
    {
        $config = $this->config->get('config');
        $site = $config->Site ?? null;
        $pda_sender = 'pda_sender_' . $realm;
        $pda_sender_name = 'pda_sender_name';
        $senderEmail = $site->$pda_sender ?? null;
        $senderName = $site->$pda_sender_name ?? null;
        return ['email' => $senderEmail, 'name' => $senderName ];
    }

    public function getPDAInstitutionRecipientData($realm)
    {
        $config = $this->config->get('config');
        $site = $config->Site ?? null;
        $pda_email = 'pda_email_' . $realm;
        $email = $site->$pda_email ?? null;
        $name = 'UB Tübingen PDA';
        return ['email' => $email, 'name' => $name];
    }

    public function sendPDAUserNotificationEmail($post, UserEntityInterface $user, $data, $id)
    {
        $userDataRaw = $this->getUserData($user);
        $userType = $userDataRaw['user_type'];
        $userData = $this->formatUserData($userDataRaw);
     ...
     */
    public function sendPDAUnsubscribeEmail(UserEntityInterface $user, $id)
    {
        $userDataRaw = $this->getUserData($user);
        $userType = $userDataRaw['user_type'];
        $userData = $this->formatUserData($userDataRaw);
     ...
     */
    public function sendPDAUserUnsubscribeEmail(UserEntityInterface $user, $id)
    {
        $userDataRaw = $this->getUserData($user);
        $userType = $userDataRaw['user_type'];
        $userData = $this->formatUserData($userDataRaw);
    ...
    public function getPDAClosing($realm)
    {
