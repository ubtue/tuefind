/**
 * origin: bootstrap5-theme (VuFind 10.1.4)
 *
 * usage:
 ...
 *
 * Important:
 * compare this JS-file with its original during every VuFind major upgrade and apply all changes that do not affect the special customizations of vufind-collapse-expand module.
 *
 */
...
VuFind.register("saveStatuses", function ItemStatuses() {
  function displaySaveStatus(itemLists, el) {
    ...
    if (itemLists.length > 0) {
      ...
      // vufind-collapse-expand: result grouping: adds first() for locating favorite selector correctly
      $item.find('.savedLists').first().addClass('loaded');
      // vufind-collapse-expand: result grouping: adds first() for locating favorite selector correctly
      $item.find('.js-load').first().replaceWith(listEl);
    } else {
      // If we got nothing back, remove the pending status:
      // vufind-collapse-expand: result grouping: adds first() for locating favorite selector correctly
      $item.find('.js-load').first().remove();
    }
